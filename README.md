...
1. **初始化应用程序**
   - 用户首次运行应用程序时，需要初始化应用程序，设置主密码，用于解密数据文件。**（请牢记密码）**
   ![示例图片](docs/init.png)
   - 初始化完成后，应用程序会自动生成几个文件：
      - `data.dat`：数据文件，用于存储用户数据。
      - `mmbox.ini`：配置文件，用于存储应用程序参数。
      - `mmbox_YYYYMMDD.log`：日志文件，用于存储应用程序运行日志。

2. **数据文件路径**
   - 初始化完成后，应用程序会自动生成一个数据文件，在当前应用程序所在目录下，文件名默认为 `data.dat`，可修改名称及路径，<span style="color: red;">请勿修改文件内容和文件类型</span>。
   - 点击“选择文件”按钮，可以自定义选择已有数据文件。

3. **私人密钥 & 解密数据 & 加载数据**
   - 私人密钥目前设定为16位，如有必要，可自行修改代码。
   - 点击“解密/加载”按钮，可以解密并加载文件数据；此过程数据在内存中进行，数据并没有持久化，且数据文件依然是密文，不用担心数据泄漏。
...
5. **表格数据操作**
   ...
   ![示例图片](docs/clickright.png)
   - 单击表格中的选中行，可以进行密码的显示隐藏。
   ![示例图片](docs/click1.png)
   - 双击表格中的选中行，可以进行编辑应用名称、账号、密码。
   ![示例图片](docs/click2.png)
